...
{
    // INI OTAKNYA (Logika PHP SQLite)
    public function simpan($uangStr, $keterangan)
    {
        // Bersihin format titik
        ...
        // Simpan ke SQLite tabel savings
        DB::table('savings')->insert([
            'user_id' => 1,
            'amount' => $amount,
            'purpose' => $keterangan ?: 'Tabungan Rutin',
            'saved_at' => now(),
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    ...
    <div x-cloak x-show="activeForm === 'nabung'" class="fixed inset-0 z-[60] flex items-end justify-center pointer-events-none">
        ...
        <div x-show="activeForm === 'nabung'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="translate-y-0" x-transition:leave-end="translate-y-full" class="relative w-full h-[95vh] bg-white rounded-t-[2.5rem] shadow-2xl pointer-events-auto flex flex-col">
            ...
            <div class="flex items-center gap-4 p-6 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-white rounded-t-[2.5rem] relative overflow-hidden">
                ...
                <button @click="backToMenu()" class="relative z-10 w-10 h-10 bg-white shadow-sm border border-emerald-100 rounded-full text-emerald-600 flex items-center justify-center hover:bg-emerald-50"><i class="fas fa-chevron-left text-lg"></i></button>
            ...
            <div class="p-6 overflow-y-auto flex-1">
                <div class="bg-gray-900 rounded-3xl p-6 mb-6 relative overflow-hidden">
                ...
                <div class="grid grid-cols-3 gap-3 mb-8">
                    <button @click="tambahUang(10000)" class="py-3 bg-emerald-50 text-emerald-600 rounded-xl font-bold text-sm">+ 10rb</button>
                    <button @click="tambahUang(20000)" class="py-3 bg-emerald-50 text-emerald-600 rounded-xl font-bold text-sm">+ 20rb</button>
                    <button @click="tambahUang(50000)" class="py-3 bg-emerald-50 text-emerald-600 rounded-xl font-bold text-sm">+ 50rb</button>
                    <button @click="tambahUang(100000)" class="py-3 bg-emerald-50 text-emerald-600 rounded-xl font-bold text-sm">+ 100rb</button>
                    <button @click="tambahUang(0, true)" class="py-3 col-span-2 bg-gray-100 text-gray-500 rounded-xl font-bold text-sm"><i class="fas fa-eraser mr-1"></i> Reset Uang</button>
                </div>
                ...
                <div class="flex gap-2 mb-3 overflow-x-auto pb-2">
                    <button @click="keteranganNabung = 'Sisa Jajan'" :class="keteranganNabung === 'Sisa Jajan' ? 'bg-gray-800 text-white shadow-md' : 'bg-white border-2 border-gray-100 text-gray-500'" class="px-5 py-2.5 rounded-full font-bold text-sm whitespace-nowrap">Sisa Jajan</button>
                    <button @click="keteranganNabung = 'Gaji Freelance'" :class="keteranganNabung === 'Gaji Freelance' ? 'bg-gray-800 text-white shadow-md' : 'bg-white border-2 border-gray-100 text-gray-500'" class="px-5 py-2.5 rounded-full font-bold text-sm whitespace-nowrap">Gaji Freelance</button>
                    <button @click="keteranganNabung = 'Jual Project'" :class="keteranganNabung === 'Jual Project' ? 'bg-gray-800 text-white shadow-md' : 'bg-white border-2 border-gray-100 text-gray-500'" class="px-5 py-2.5 rounded-full font-bold text-sm whitespace-nowrap">Jual Project</button>
                </div>
            ...
            <div class="p-6 border-t border-gray-50 bg-white">
                <button @click="$wire.simpan(uangStr, keteranganNabung).then(() => { backToMenu(); tambahUang(0, true); keteranganNabung = ''; })" class="w-full bg-emerald-500 text-white font-bold text-lg py-4 rounded-2xl flex justify-center items-center gap-2 transition-all active:scale-95">
